...
We do not provide a graph/code to deploy a model. However, we have cobbled together a notebook ([predict.ipynb](https://github.com/untrix/im2latex/blob/master/src/predict.ipynb)) to run a model snapshot against a set of images and output results to a csv file.
