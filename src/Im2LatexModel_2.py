...
class Im2LatexModel(tf.nn.rnn_cell.RNNCell):
        ...
        with tf.variable_scope('Im2LatexModel') as scope:
            ...
            if len(self.C.D_RNN) == 1:
                ...
                self._CALSTM_stack = tf.nn.rnn_cell.MultiRNNCell(
                        [CALSTM(rnn_params, self._a, beamsearch_width, reuse) 
            ...
            with tf.variable_scope('Initializer_MLP'):
                ...
                def zero_to_init_state(zs, counter):
                    ...
                    if self._num_calstm_layers == 1:
                        ...
                        cs = CALSTM.init_state(cs, counter, self.C.init_model, a)
                    else:
                        ...
                        for i in xrange(len(cs)):
                            ...
                            lst.append(CALSTM.init_state(cs[i], counter, self.C.init_model, a))
    ...
    def _scan_step_training(self, out_t_1, x_t):
        ...
        return ScanOut(yLogits_t, state_t)
    
    def build_train_graph(self):
        ## y_s is the batch of target word sequences
        y_s = tf.placeholder(dtype=self.C.dtype, shape=(self.C.B, None), name='Y_s') # (B, T)
        ...
        with tf.variable_scope('training_graph'):
            ...
            x_s = K.concatenate((self._x_0, y_s[0:-2]), axis=0)
            
            """ Build the training graph of the model """
            accum = (None, self.init_state())
            ...
            yLogits_s, alpha_s = out_s.yLogits, out_s.state.calstm_state.alpha
            ...
            alpha = K.permute_dimensions(alpha_s, [1,0,2])
            
            return self._optimizer(yLogits, y_s, alpha, seq_lens).updated({'y_s': y_s, 
                                                                           'seq_lens': seq_lens,
                                                                           'im': self._im})
        ...
        def _optimizer(self, yLogits, y_s, alpha, sequence_lengths):
            ...
            with tf.variable_scope('Optimizer'):
                global_step = tf.get_variable('global_step', shape=0, dtype=tf.int32, trainable=False, initializer=0)
