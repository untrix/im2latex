...
def measure_accuracy(session, ops, batch_its, hyper, args, step, tf_sw):
    ...
    if not validate:
        if args.make_training_accuracy_graph: # Run one training batch through training_accuracy_graph
            ...
            tf_sw.add_summary(logs_top1, log_step(step))
            tf_sw.add_summary(logs_topK, log_step(step))
            tf_sw.flush()
            metrics = dlc.Properties({
                'valid_time_per100': ((time.time() - valid_start_time) * 100. / batch_its.tr_acc_it.batch_size)
                })
